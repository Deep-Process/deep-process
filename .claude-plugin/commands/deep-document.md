...
This command is part of the `deep-process` plugin. The process files are in the `processes/deep-document/` directory at the plugin root (one level up from this command file's `commands/` directory).
